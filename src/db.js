  ...
  if (rows[0].c === 0) {
    ...
    await pool.query(
      `insert into users (personal_number, full_name, phone, email, password_hash, is_admin, consent_at)
       values ($1,$2,$3,$4,$5,true, now()), ($6,$7,$8,$9,$10,false, now())`,
    ...
    await pool.query(
      `insert into shifts (user_id,date,start_time,end_time,site,role,notes) values
       ($1, current_date+1, '06:00','14:00','Provoz A','Kuchyň',''),
       ($1, current_date+2, '14:00','22:00','Provoz B','Pokladna','')`,
      [u1.rows[0].id]
    );
  }
}
