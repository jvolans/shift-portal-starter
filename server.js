...
// Admin + import směn (WorkCenter místo Site)
...
const uploadMw = upload.single('csvfile');
app.post('/admin/import-shifts', requireAuth, requireAdmin, uploadMw, async (req, res) => {
  try {
    ...
    try {
      for (const r of records) {
        ...
        const workCenter = r.WorkCenter ?? r.Site ?? '';

        const ex = (await pool.query(
          'select id from shifts where user_id=$1 and date=$2 and start_time=$3 and end_time=$4',
          [u.id, r.Date, r.Start, r.End]
        )).rows[0];

        if (ex) {
          await pool.query(
            'update shifts set date=$1, start_time=$2, end_time=$3, site=$4, notes=$5, updated_at=now() where id=$6',
            [r.Date, r.Start, r.End, workCenter, r.Notes || '', ex.id]
          );
          updated++;
        } else {
          await pool.query(
            'insert into shifts (user_id, date, start_time, end_time, site, notes) values ($1,$2,$3,$4,$5,$6)',
            [u.id, r.Date, r.Start, r.End, workCenter, r.Notes || '']
          );
          created++;
        }
      }
...
// (Volitelně: přidání nového uživatele – můžeš nechat nebo odebrat)
